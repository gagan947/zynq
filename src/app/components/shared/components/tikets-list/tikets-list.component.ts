...
export class TiketsListComponent {
  ...
  searchTickets(event: Event) {
    ...
    if (searchTerm) {
      this.tickets = this.originalTickets.filter(ticket =>
        ticket.issue_title.toLowerCase().includes(searchTerm) && ticket.created_at.includes(this.date.nativeElement.value)
      );
  ...
  filterByDate(event: Event) {
    ...
    if (date) {
      this.tickets = this.originalTickets.filter(ticket => ticket.created_at.includes(date) && (ticket.issue_title.toLowerCase().includes(this.search.nativeElement.value.toLowerCase())));
    } else if (this.search.nativeElement.value) {
      this.tickets = this.originalTickets.filter(ticket => ticket.issue_title.toLowerCase().includes(this.search.nativeElement.value.toLowerCase()))
    } else {
      this.tickets = [...this.originalTickets];
    }
  }
